...
from .models import (
    ...
    Designation,
    Department,
    Additional_library_information,
    Media,
)
...
class AddLibraryForm(forms.ModelForm):
    ...
    library_phone = forms.CharField(
        max_length=300,
        widget=forms.TextInput(
            attrs={"class": "form-control", "placeholder": "Tel"}
        ),
    )

    library_country = CountryField().formfield(
...
class UpdateLibraryForm(forms.ModelForm):
    ...
    class Meta:
        model = Library
        exclude = ("library_admin", "plan", "paid_until", "is_still_active")
        widgets = {
            "library_country": CountrySelectWidget(),
            "class": "form-control",
        }
